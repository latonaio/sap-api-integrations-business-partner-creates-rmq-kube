...
import (
	"fmt"
	sap_api_caller "sap-api-integrations-business-partner-creates-rmq-kube/SAP_API_Caller"
	"sap-api-integrations-business-partner-creates-rmq-kube/SAP_API_Caller/requests"
	sap_api_input_reader "sap-api-integrations-business-partner-creates-rmq-kube/SAP_API_Input_Reader"
	"sap-api-integrations-business-partner-creates-rmq-kube/config"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	general, role, address, bank := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncPostBusinessPartner(address, bank, general, role, accepter)
	return nil
}
...
func extractData(data map[string]interface{}) (
	...
	role = sdc.ConvertToRole()
	address = sdc.ConvertToAddress()
	bank = sdc.ConvertToBank()
	return general, role, address, bank
}
